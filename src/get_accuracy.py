...
if __name__ == "__main__":
    ...
    with open('../results/classification/processed/'+model+'/superior_model','w') as writer:
        csv_data = ''
        for item in superior:
            csv_data += str(item)+','
        writer.writelines(csv_data[:-1])
        
    with open('../results/classification/processed/'+model+'/inferior_model','w') as writer:
        csv_data = ''
        for item in inferior:
            csv_data += str(item)+','
        writer.writelines(csv_data[:-1])
